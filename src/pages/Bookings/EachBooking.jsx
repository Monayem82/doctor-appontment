...
const EachBooking = ({ appointment, doctorsSummaryShort }) => {
                ...
                <div className="flex flex-col md:flex-row items-start p-6">
                    ...
                    <div className="md:w-3/4 w-full md:pl-6 pt-4 md:pt-0 space-y-4">
                        ...
                        <div className="flex justify-between items-center pb-2 border-b border-dashed border-gray-300">
                            <h3 className="text-xl font-extrabold text-base-content">Appointment Details</h3> {/* অ্যাপয়েন্টমেন্ট ডিটেইলস */}
                            <div className="badge badge-success text-white font-bold text-sm">Confirmed</div> {/* কনফার্মড */}
                        </div>
                        ...
                        <div className="flex items-center space-x-6">
                            <div className="flex items-center space-x-2">
                                ...
                                <div>
                                    <p className="text-xs text-gray-500">Date</p> {/* তারিখ */}
                                    <p className="font-bold text-lg">{appointment.date || '2025-12-25'}</p> {/* Use actual appointment date */}
                                </div>
                            </div>
                            <div className="flex items-center space-x-2">
                                ...
                                <div>
                                    <p className="text-xs text-gray-500">Time</p> {/* সময় */}
                                    <p className="font-bold text-lg">{appointment.time || '10:00 AM'}</p> {/* সকাল ১০:০০ টা */}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-2">
                            <p className="font-semibold text-base-content">Chamber Address:</p> {/* চেম্বারের ঠিকানা: */}
                            <p className="text-sm font-medium">{seating_location.hospital_name}</p>
                            <p className="text-xs text-gray-600">{seating_location.address} (Room: {seating_location.room_number})</p> {/* (রুম: {seating_location.room_number}) */}
                        </div>

                        {/* Total Fee and Action */}
                        <div className="flex justify-between items-center pt-3 border-t">
                            <div className="text-left">
                                <p className="font-semibold text-base mb-1">Total Fee Paid:</p> {/* মোট ফি পরিশোধ: */}
                                <p className="text-2xl font-extrabold text-success">BDT {consultation_details.total_fee_bdt}</p>
                                <p className="text-xs text-gray-500">Including VAT+Tax</p> {/* VAT+Tax সহ */}
                            </div>
                            <div className="card-actions">
                                {/* You can add a button for cancellation or viewing details */}
                                <button onClick={() => handleRemoveAppointment(id)} className="btn btn-outline btn-error btn-sm">Cancel</button> {/* ক্যান্সেল */}
